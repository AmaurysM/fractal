...
export const SettingsSidebar = ({
  ...
  return (
    <>
      {/* ── MOBILE: horizontal tab bar + section pills ── */}
      <div className="flex flex-col border-b border-[#2a2b2c] bg-[#1e1f20] sm:hidden shrink-0">
        {/* Search */}
            ...
            return (
              <button
                ...
                className={`
                  ...
                  font-medium whitespace-nowrap transition-colors duration-100 cursor-pointer
                  ${isSelected
                    ? "bg-[#1f2937] text-[#6a9fd8]"
                    : "text-[#666] hover:text-[#999] hover:bg-[#252627]"
                  }
                `}
              >
                <span className="font-mono text-[10px] opacity-70">{TAB_ICONS[tab]}</span>
        ...
        {/* Section pills for open tab */}
        {openTab && (
          <div className="flex gap-1.5 px-3 pb-2 overflow-x-auto">
            {TAB_SECTIONS[openTab].map((section) => {
              const isActive = activeSection === section.label && selectedTab === openTab;
              return (
                <button
                  key={section.label}
                  onClick={() => handleSectionClick(openTab, section.label)}
                  className={`
                    px-3 py-1 rounded-full text-[10px] whitespace-nowrap
                    transition-colors duration-100 cursor-pointer capitalize tracking-wide
                    ${isActive
                      ? "bg-[#2a3a4a] text-[#6a9fd8] border border-[#6a9fd8]/30"
                      : "text-[#555] border border-[#2a2b2c] hover:text-[#888] hover:border-[#3a3b3c]"
                    }
                  `}
                >
                  {section.label}
      ...
      <aside className="hidden sm:flex flex-col border-r border-[#2a2b2c] h-full w-52 shrink-0 bg-[#1e1f20]">
        <div className="flex items-center gap-2 px-3 py-2 border-b border-[#2a2b2c]">
          <CiSearch className="text-[#666] shrink-0" size={14} />
          <input
            type="search"
